...
use lawkit_core::{law as core_law, LawkitOptions, LawkitSpecificOptions, OutputFormat, LawkitResult};
use regex::Regex;
...
/// ```
#[napi]
pub fn law(subcommand: String, data_or_config: serde_json::Value, options: Option<JsLawkitOptions>) -> Result<Vec<JsLawkitResult>> {
    // Convert options
    let rust_options = options.map(|opts| build_lawkit_options(opts)).transpose()?;
    ...
    let js_results = results.into_iter()
        .map(|result| convert_lawkit_result(result))
...
fn convert_lawkit_result(result: LawkitResult) -> Result<JsLawkitResult> {
    ...
    match result {
        ...
        LawkitResult::GeneratedData(path, data) => {
            ...
            js_result.parameters = Some(serde_json::to_value(data.parameters).unwrap_or(serde_json::Value::Null));
