...
fn output_cross_validation_result(
    ...
    writeln!(writer, "Stability Assessment: {:?}", result.stability_assessment)?;
    writeln!(writer)?;
...
fn output_consistency_check_result(
    ...
    writeln!(writer)?;
    writeln!(writer, "Dataset: {}", result.dataset_name)?;
    writeln!(writer, "Threshold: {:.3}", threshold)?;
    ...
    if result.consistency_score < threshold {
        ...
        writeln!(writer, "Recommendation: Review data quality and collection methods")?;
    } else {
