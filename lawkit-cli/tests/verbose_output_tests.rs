...
mod verbose_output_tests {
    ...
    fn run_lawkit_verbose(subcommand: &str, args: &[&str], input_data: Option<&str>) -> std::process::Output {
    ...
    fn test_verbose_consistency_across_commands() {
        ...
        for command in &commands {
            ...
            assert!(stderr.contains("Debug: input argument = None"), 
                "Command {} missing input argument debug", command);
            assert!(stderr.contains("Debug: Reading from stdin"), 
                "Command {} missing stdin debug", command);
        }
    }
