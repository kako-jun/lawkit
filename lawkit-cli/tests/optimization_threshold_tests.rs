...
fn test_normal_above_threshold_uses_streaming() {
    ...
    assert!(stderr.contains("Streaming analysis successful") || stderr.contains("chunks processed"));
}
...
fn run_lawkit_command_with_debug(subcommand: &str, args: &[&str]) -> std::process::Output {
    ...
    if !test_data.is_empty() {
        ...
        if let Some(stdin) = child.stdin.take() {
            use std::io::Write;
            let mut stdin = stdin;
            stdin.write_all(test_data.as_bytes()).expect("Failed to write to stdin");
        }
