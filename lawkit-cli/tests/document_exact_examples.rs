...
fn test_readme_basic_benf() {
    ...
    assert!(matches!(
        output.status.code(),
        Some(0) | Some(10) | Some(11) | Some(12) | Some(13)
    ));
...
fn test_readme_timeseries() {
    ...
    let output = run_lawkit_command_with_file(
        "normal",
        &monthly_data,
        &["--enable-timeseries", "--timeseries-window", "12"],
    );
...
fn test_readme_generate_pipeline() {
    ...
    let analyze_output = run_lawkit_command(
        "analyze",
        &[temp_file.path().to_str().unwrap(), "--laws", "all"],
    );
    assert!(matches!(
