        ...
        if test_path.exists() {
            ...
            match result {
                Ok(numbers) => {
                    ...
                    assert!(numbers.contains(&567.89), "Should contain decimal amount 567.89");
                    assert!(numbers.contains(&234.0), "Should contain partial amount 234");
                    ...
                    assert!(numbers.len() >= 30, "Should extract at least 30 numbers, got {}", numbers.len());
            ...
            println!("Test Word file not found at {:?}, skipping real file test", test_path);
        }
    }
}