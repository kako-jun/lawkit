...
impl NormalResult {
    pub fn new(dataset_name: String, numbers: &[f64]) -> Result<Self> {
        ...
        let normality_score = calculate_normality_score(
            shapiro_result.1, anderson_result.1, ks_result.1, qq_correlation
        );
        let distribution_quality = calculate_distribution_quality(skewness, kurtosis, normality_score);
        ...
        // リスクレベル判定
        let risk_level = determine_risk_level(normality_score, &outliers_z_score, skewness, kurtosis);
...
fn shapiro_wilk_test(numbers: &[f64]) -> (f64, f64) {
    ...
    // 簡易実装: 分位数相関係数を使用
    let qq_corr = calculate_qq_correlation(&sorted, calculate_mean(&sorted), calculate_variance(&sorted, calculate_mean(&sorted)).sqrt());
...
fn standard_normal_cdf(z: f64) -> f64 {
    ...
    let probability = d * t * (0.3193815 + t * (-0.3565638 + t * (1.7814779 + t * (-1.8212560 + t * 1.3302744))));
...
fn calculate_distribution_quality(skewness: f64, kurtosis: f64, normality_score: f64) -> f64 {
    ...
    (normality_score * 0.5 + skew_score * 0.25 + kurt_score * 0.25).max(0.0).min(1.0)
}
...
/// リスクレベル判定
fn determine_risk_level(normality_score: f64, outliers: &[(usize, f64, f64)], skewness: f64, kurtosis: f64) -> RiskLevel {
    ...
    if normality_score > 0.8 && outlier_ratio < 0.05 && skewness.abs() < 0.5 && kurtosis.abs() < 0.5 {
        RiskLevel::Low
    } else if normality_score > 0.6 && outlier_ratio < 0.1 && skewness.abs() < 1.0 && kurtosis.abs() < 1.0 {
        RiskLevel::Medium
...
mod tests {
    ...
    fn test_normal_distribution() {
        // 標準正規分布に近いデータ
        let numbers = vec![
            0.0, 0.5, -0.3, 1.2, -0.8, 0.2, -0.1, 0.9, -0.6, 0.7,
            -0.4, 0.3, 1.1, -0.9, 0.6, -0.2, 0.8, -0.5, 0.1, 0.4
        ];
