...
pub fn analyze_rare_events(numbers: &[f64], lambda: f64) -> RareEventAnalysis {
    ...
    let extreme_events: Vec<ExtremeEvent> = event_counts.iter().enumerate()
        ...
        .map(|(index, &count)| ExtremeEvent {
            ...
            rarity_level: if count >= threshold_999 { RarityLevel::ExtremelyRare }
                         else if count >= threshold_99 { RarityLevel::VeryRare }
                         else { RarityLevel::Rare },
        })
        .collect();
...
pub fn analyze_time_intervals(intervals: &[f64]) -> Result<TimeIntervalAnalysis> {
    ...
    Ok(TimeIntervalAnalysis {
        ...
        is_poisson_process: exponential_fit > 0.05 && memoryless_test > 0.05 && homogeneity_test > 0.05,
        confidence_interval_lambda: calculate_lambda_ci_from_intervals(lambda_estimate, intervals.len()),
    })
}
...
fn test_exponential_fit(intervals: &[f64], mean_interval: f64) -> f64 {
    ...
    if max_diff > critical { 0.01 } else { 0.1 }
}
...
fn test_memoryless_property(intervals: &[f64]) -> f64 {
    ...
    if abs_correlation < mean * 0.1 { 0.1 } else { 0.01 }
}
...
fn test_homogeneity(intervals: &[f64]) -> f64 {
    ...
    // 比が1に近いほど斉次
    if (ratio - 1.0).abs() < 0.2 { 0.1 } else { 0.01 }
}
