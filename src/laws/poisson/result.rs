...
impl PoissonResult {
    pub fn new(dataset_name: String, numbers: &[f64]) -> Result<Self> {
        ...
        let expected_frequencies = calculate_expected_frequencies(lambda, numbers.len(), &frequency_distribution);
        
        // 適合度検定
        let (chi_square_statistic, chi_square_p_value) = chi_square_goodness_of_fit_test(
            &frequency_distribution, &expected_frequencies
        );
        
        let (ks_statistic, ks_p_value) = kolmogorov_smirnov_poisson_test(&event_counts, lambda);
        
        // 適合度評価
        let goodness_of_fit_score = calculate_goodness_of_fit_score(
            variance_ratio, chi_square_p_value, ks_p_value
        );
        let poisson_quality = calculate_poisson_quality_score(variance_ratio, sample_mean);
        let distribution_assessment = assess_poisson_distribution(goodness_of_fit_score, variance_ratio);
        ...
        let rare_events_count = event_counts.iter()
        ...
        // 信頼区間
        let confidence_interval_lambda = calculate_lambda_confidence_interval(sample_mean, numbers.len());
