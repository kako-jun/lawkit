...
fn test_analyze_all_laws() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = lawkit_cmd();
    cmd.arg("analyze").arg("--laws").arg("all").arg(temp_file.path());
    cmd.assert()
        .success()
        .stdout(predicates::str::contains("Statistical Laws Integration Analysis"));
...
fn test_generate_sample_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = lawkit_cmd();
    cmd.arg("generate").arg("--law").arg("benf").arg("--count").arg("100");
    cmd.assert()
        .success();
...
fn test_analyze_specific_laws() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = lawkit_cmd();
    cmd.arg("analyze").arg("--laws").arg("benf,pareto").arg(temp_file.path());
    cmd.assert()
        .success();
...
fn test_laws_executed_count() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = lawkit_cmd();
    cmd.arg("analyze").arg("--laws").arg("benf,pareto,zipf").arg(temp_file.path());
    cmd.assert()
        .success()
...
fn test_help_command() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicates::str::contains("Statistical law analysis toolkit"));
