use lawkit_core::laws::benford::{
    // analyze_benford_law, 
    calculate_digit_distribution, 
    get_first_digit,
    BENFORD_EXPECTED_PERCENTAGES
};
use lawkit_core::common::statistics::calculate_chi_square;
...
mod benford_distribution_tests {
    ...
    fn test_benford_expected_percentages_constant() {
        ...
        let expected = [30.103, 17.609, 12.494, 9.691, 7.918, 6.695, 5.799, 5.115, 4.576];
        // assert_slices_approx_eq(
        //     &BENFORD_EXPECTED_PERCENTAGES,
        //     &expected,
        //     0.001,
        ...
        let sum: f64 = lawkit_core::laws::benford::analysis::BENFORD_EXPECTED_PERCENTAGES.iter().sum();
        assert!((sum - 100.0).abs() < 0.001, "Expected percentages should sum to 100%, got {}", sum);
    }

    #[test]
    fn test_extract_first_digits() {
        let numbers = vec![123.45, 234.56, 345.67, 456.78];
        // let first_digits = extract_first_digits(&numbers);
    ...
    fn test_extract_first_digits_edge_cases() {
        ...
        // let first_digits = extract_first_digits(&numbers);
        // Zero and numbers starting with zero should be filtered
        ...
        // Test with decimal numbers less than 1
    ...
    fn test_calculate_first_digit_distribution() {
        let first_digits = vec![1, 1, 1, 2, 2, 3];
        // let distribution = calculate_first_digit_distribution(&first_digits);
        ...
        // assert_slices_approx_eq(&distribution, &expected, 0.001, "First digit distribution");
    }

    #[test]
    fn test_calculate_first_digit_distribution_empty() {
        let first_digits: Vec<u8> = vec![];
        // let distribution = calculate_first_digit_distribution(&first_digits);
        
        // Should be all zeros for empty input
        let expected = [0.0; 9];
        // assert_slices_approx_eq(&distribution, &expected, 0.001, "Empty distribution");
    }
    ...
    fn test_chi_square_calculation() {
        ...
        let chi_square = lawkit_core::common::statistics::calculate_chi_square(&observed, &expected);
        
        // Chi-square should be approximately 0 for identical distributions
        assert!(chi_square < 0.001, "Chi-square should be near 0 for identical distributions, got {}", chi_square);
    }
    ...
    fn test_chi_square_with_deviation() {
        ...
        assert!(chi_square > 10.0, "Chi-square should be high for skewed distribution, got {}", chi_square);
        
        // P-value should be low (significant deviation)
        assert!(p_value < 0.05, "P-value should be low for significant deviation, got {}", p_value);
    }
...
mod integration_benford_tests {
    ...
    fn test_full_benford_analysis_pipeline() {
        ...
        let test_numbers = vec![
            123.0, 234.0, 345.0, 456.0, 567.0, 678.0, 789.0, 890.0, 901.0,
