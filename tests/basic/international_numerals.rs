...
use lawkit_core::common::international::{
    convert_international_numerals, 
    extract_numbers_international
};
...
mod chinese_numerals_tests {
    ...
    fn test_chinese_financial_complex() {
        ...
        for (chinese, expected) in test_cases {
            ...
            assert!(result.contains(expected), "Failed for {}: expected {}, got {}", chinese, expected, result);
        }
    }
...
mod mixed_numerals_tests {
    ...
    fn test_mixed_script_conversion() {
        ...
        assert!(count >= 3, "Should find at least 3 instances of '123', found {}", count);
    }
    ...
    fn test_mixed_numerals_comprehensive() {
        ...
        assert!(count_100 >= 2, "Should find at least 2 instances of 100, found {}", count_100);
    }
}
