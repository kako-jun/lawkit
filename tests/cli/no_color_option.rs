    ...
    if let Ok(test_file) = create_test_csv() {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.path().to_str().unwrap(), "--no-color"])
        ...
        assert!(!stdout.contains("\x1b["), "Output should not contain ANSI color codes when --no-color is specified");
        assert!(stdout.contains("Low") || stdout.contains("Medium") || stdout.contains("High") || stdout.contains("Critical"));
    }
    Ok(())
}
...
fn test_generate_benf_no_color_option() {
    let output = Command::new("cargo")
        .args(["run", "--bin", "lawkit", "--", "generate", "benf", "--samples", "10", "--no-color"])
        .current_dir(env!("CARGO_MANIFEST_DIR"))
        .output()
...
fn test_color_vs_no_color_difference() -> Result<(), Box<dyn std::error::Error>> {
    if let Ok(test_file) = create_test_csv() {
        ...
        assert!(!stdout_no_color.contains("\x1b["), "No-color output should not contain ANSI codes");
        
        // Both should contain the same semantic content
        assert!(stdout_color.contains("Low") || stdout_color.contains("Medium") || 
                stdout_color.contains("High") || stdout_color.contains("Critical"));
        assert!(stdout_no_color.contains("Low") || stdout_no_color.contains("Medium") || 
                stdout_no_color.contains("High") || stdout_no_color.contains("Critical"));
    }
    Ok(())
}