        ...
        for format in ["text", "csv", "json", "yaml", "toml", "xml"] {
            let output = Command::new("cargo")
                .args(["run", "--bin", "lawkit", "--", "benf", 
                       test_file.to_str().unwrap(), "--format", format])
...
fn test_benf_quiet_verbose_options() {
    if let Ok(test_file) = create_test_csv() {
        ...
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.to_str().unwrap(), "--verbose"])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
        ...
        if output.status.success() {
            assert!(stdout_verbose.len() >= stdout_quiet.len() || 
...
fn test_benf_filter_option() {
    if let Ok(test_file) = create_test_csv() {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.to_str().unwrap(), "--filter", ">=100"])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
...
fn test_benf_confidence_option() {
    if let Ok(test_file) = create_test_csv() {
        for confidence in ["0.90", "0.95", "0.99"] {
            let output = Command::new("cargo")
                .args(["run", "--bin", "lawkit", "--", "benf", 
                       test_file.to_str().unwrap(), "--confidence", confidence])
                .current_dir(env!("CARGO_MANIFEST_DIR"))
                .output()
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                assert!(!stderr.contains("unrecognized"), 
                        "Confidence {} should be recognized", confidence);
            }
        }
    }
}
...
fn test_benf_threshold_option() {
    if let Ok(test_file) = create_test_csv() {
        for threshold in ["low", "medium", "high", "critical", "auto"] {
            let output = Command::new("cargo")
                .args(["run", "--bin", "lawkit", "--", "benf", 
                       test_file.to_str().unwrap(), "--threshold", threshold])
                .current_dir(env!("CARGO_MANIFEST_DIR"))
                .output()
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                assert!(!stderr.contains("unrecognized"), 
                        "Threshold {} should be recognized", threshold);
            }
        }
    }
}
...
fn test_benf_min_count_option() {
    if let Ok(test_file) = create_test_csv() {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.to_str().unwrap(), "--min-count", "5"])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
...
fn test_benf_sample_size_option() {
    if let Ok(test_file) = create_test_csv() {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.to_str().unwrap(), "--sample-size", "50"])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
...
fn test_benf_min_value_option() {
    if let Ok(test_file) = create_test_csv() {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.to_str().unwrap(), "--min-value", "100"])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
...
fn test_main_commands_help() {
    let commands = ["benf", "pareto", "zipf", "normal", "poisson", 
                   "analyze", "validate", "diagnose", "generate", "list", "selftest"];
    ...
    for command in &commands {
        ...
        assert!(output.status.success(), 
                "Command {} --help should succeed", command);
        
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(!stdout.is_empty(), 
                "Command {} --help should produce output", command);
    }
}
