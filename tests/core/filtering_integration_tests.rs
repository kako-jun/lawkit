...
fn test_risk_threshold_integration() {
    let test_numbers = vec![
        111.0, 222.0, 333.0, 444.0, 555.0, 666.0, 777.0, 888.0, 999.0, 1000.0,
    ];

    // Test auto threshold
    let auto_threshold = RiskThreshold::Auto;
    let result =
        BenfordResult::new_with_threshold("test".to_string(), &test_numbers, &auto_threshold, 5)
            .unwrap();
    ...
    let result_low =
        BenfordResult::new_with_threshold("test".to_string(), &test_numbers, &low_threshold, 5)
            .unwrap();
...
fn test_minimum_count_requirement() {
    ...
    let result_small = BenfordResult::new_with_threshold(
        "small".to_string(),
        &small_dataset,
        &RiskThreshold::Auto,
        5,
    ...
    let result_large = BenfordResult::new_with_threshold(
        "large".to_string(),
        &large_dataset,
        &RiskThreshold::Auto,
        5,
    ...
    let result_small_min = BenfordResult::new_with_threshold(
        "small".to_string(),
        &small_dataset,
        &RiskThreshold::Auto,
        2,
...
fn test_filter_with_benford_analysis() {
    ...
    let numbers = vec![
        111.0, 122.0, 133.0, 144.0, 155.0, 166.0, 177.0, 188.0, 199.0, 200.0, 211.0, 222.0, 233.0,
        244.0, 255.0, 266.0, 277.0, 288.0, 299.0, 300.0, 311.0, 322.0, 333.0, 444.0, 555.0, 666.0,
        777.0, 888.0, 999.0, 1000.0,
    ];
    ...
    let result = BenfordResult::new_with_threshold(
        "filtered".to_string(),
        &filtered_numbers,
        &RiskThreshold::Auto,
        1,
...
fn test_custom_threshold_parsing() {
    ...
    assert_eq!(
        RiskThreshold::from_str("auto").unwrap(),
        RiskThreshold::Auto
    );
    assert_eq!(RiskThreshold::from_str("low").unwrap(), RiskThreshold::Low);
    assert_eq!(
        RiskThreshold::from_str("medium").unwrap(),
        RiskThreshold::Medium
    );
    assert_eq!(
        RiskThreshold::from_str("high").unwrap(),
        RiskThreshold::High
    );
    assert_eq!(
        RiskThreshold::from_str("critical").unwrap(),
        RiskThreshold::Critical
    );

    // Test custom p-value
    assert_eq!(
        RiskThreshold::from_str("0.03").unwrap(),
        RiskThreshold::Custom(0.03)
    );
