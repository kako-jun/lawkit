...
fn test_comprehensive_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=1000 {
        ...
        let revenue = if i <= 100 { 1000000 / i } else { 10000 + i * 50 };
    ...
    assert!(detected_laws >= 2, "Should detect at least 2 statistical laws, found {}", detected_laws);
...
fn test_data_generation_all_laws() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for law in &laws {
        ...
        assert!(content.lines().count() > 10, "{} data has too few lines", law);
    }
...
fn test_confidence_level_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=200 {
        ...
        let first_digit = if i <= 60 { 1 }      // Too many 1s
                         else if i <= 90 { 2 }   // Some 2s
                         else if i <= 110 { 3 }  // Some 3s  
                         else { (i % 7) + 1 };   // Rest distributed
    ...
    for confidence in ["0.90", "0.95", "0.99"] {
        ...
        let output = cmd.output()?;
        assert!(output.status.success(), "Failed with confidence level {}", confidence);
...
fn test_large_dataset_optimization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(duration.as_secs() < 30, "Large dataset analysis took too long: {:?}", duration);
