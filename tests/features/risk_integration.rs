...
fn test_benf_risk_assessment_low() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(stdout.contains("Low") || stdout.contains("Medium") || 
            stdout.contains("High") || stdout.contains("Critical"),
            "Output should contain risk level assessment");
    
    // Exit code should be appropriate for risk level
    let exit_code = output.status.code().unwrap_or(0);
    assert!(exit_code >= 0 && exit_code <= 11, 
...
fn test_benf_risk_assessment_json_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !stdout.trim().is_empty() {
        ...
        assert!(json_str.contains("risk") || json_str.contains("Risk") ||
               json_str.contains("Low") || json_str.contains("Medium") ||
               json_str.contains("High") || json_str.contains("Critical"),
...
fn test_risk_assessment_consistency_across_commands() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for command in &commands {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", command, 
                   test_file.path().to_str().unwrap()])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
            .expect(&format!("Failed to execute lawkit {} for risk assessment", command));

        // All commands should execute successfully
        assert!(output.status.success() || output.status.code().unwrap_or(255) <= 11,
               "Command {} should execute successfully or return valid risk exit code", command);
        
        let stdout = String::from_utf8_lossy(&output.stdout);
        
        // Each command should provide some form of analysis output
        assert!(!stdout.trim().is_empty(), 
               "Command {} should produce output", command);
    }
...
fn test_analyze_comprehensive_risk_assessment() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(stdout.contains("risk") || stdout.contains("confidence") || 
...
fn test_validate_data_quality_risk() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(stdout.contains("validation") || stdout.contains("quality") ||
           stdout.contains("consistent") || stdout.contains("valid"),
...
fn test_diagnose_conflict_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = Command::new("cargo")
        .args(["run", "--bin", "lawkit", "--", "diagnose", 
               test_file.path().to_str().unwrap(), "--laws", "benf,normal"])
        .current_dir(env!("CARGO_MANIFEST_DIR"))
        .output()
    ...
    assert!(stdout.contains("diagnose") || stdout.contains("conflict") ||
           stdout.contains("recommendation") || stdout.contains("consistent"),
...
fn test_exit_codes_risk_mapping() -> Result<(), Box<dyn std::error::Error>> {
    let test_files = [
        create_normal_data_csv()?,
        create_suspicious_data_csv()?,
    ];
    ...
    for (i, test_file) in test_files.iter().enumerate() {
        ...
        assert!(exit_code == 0 || exit_code == 10 || exit_code == 11,
...
fn test_risk_threshold_option() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for threshold in &thresholds {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.path().to_str().unwrap(), "--threshold", threshold])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
            .expect(&format!("Failed to execute lawkit benf with threshold {}", threshold));

        // Should execute successfully regardless of threshold
        assert!(output.status.success() || output.status.code().unwrap_or(255) <= 11,
               "Command with threshold {} should execute successfully", threshold);
        
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(!stdout.trim().is_empty(), 
               "Should produce output with threshold {}", threshold);
    }
...
fn test_confidence_level_risk_correlation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for confidence in &confidence_levels {
        let output = Command::new("cargo")
            .args(["run", "--bin", "lawkit", "--", "benf", 
                   test_file.path().to_str().unwrap(), "--confidence", confidence])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
            .expect(&format!("Failed to execute lawkit benf with confidence {}", confidence));

        assert!(output.status.success() || output.status.code().unwrap_or(255) <= 11,
               "Command with confidence {} should execute successfully", confidence);
        
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(!stdout.trim().is_empty(), 
               "Should produce output with confidence level {}", confidence);
    }
