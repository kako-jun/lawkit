...
fn test_gini_coefficient_calculation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stdout.contains("gini") 
        || stdout.contains("inequality")
        || stdout.contains("coefficient")
    );
...
fn test_multi_column_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for region in ["North", "South", "East", "West"] {
        for i in 1..=25 {
            let sales = if region == "North" && i <= 5 { 50000 + i * 10000 } else { 5000 + i * 100 };
            let profit = sales / 5;
...
fn test_inventory_management_scenario() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=20 {
        csv_content.push_str(&format!("SKU{},{},{},{}\n", i, 10000 - i * 200, 500 + i * 10, 1000 + i * 20));
    }
    
    // B items: 30% of SKUs, 15% of value  
    for i in 21..=50 {
        csv_content.push_str(&format!("SKU{},{},{},{}\n", i, 2000 - (i - 20) * 30, 200 + i * 5, 300 + i * 8));
    }
    
    // C items: 50% of SKUs, 5% of value
    for i in 51..=100 {
        csv_content.push_str(&format!("SKU{},{},{},{}\n", i, 100 + (i - 50) * 2, 50 + i, 100 + i * 2));
    }
