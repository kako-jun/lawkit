...
### 🚀 **重大仕様変更: lawkit統合プラットフォーム化** (2025-07-02)

#### 背景と課題
- **調査結果**: ベンフォードの法則以外に10個以上の不正検知用自然法則が存在
- **問題点**: 
  - 個別コマンド実装では重複コード増大（入力処理、国際数字、多言語出力）
  - マイナーな法則の埋没・認知度低下
  - ユーザーが複数ツール習得の負担
  - メンテナンス・品質保証の非効率

#### 新アーキテクチャ: lawkit統合プラットフォーム
```bash
# 新しいコマンド体系
lawkit benf data.xlsx              # ベンフォードの法則
lawkit zipf text.txt               # ジップの法則
lawkit newcomb elections.csv       # ニューカムの法則
lawkit pareto income.xlsx          # パレートの法則
...
#### 実装方針
- **Gitリポジトリ継承**: 既存履歴・タグ・issueを活用
- **段階的移行**: 
  1. benfサブコマンド化（既存機能100%互換）
  2. 新法則の逐次追加
  3. 統合機能（比較・組み合わせ解析）の実装
- **コード再利用**: 
  - 国際数字処理エンジン（5文字体系対応）
  - 多言語出力システム（5言語対応）
  - ファイル形式パーサー（Excel/Word/PDF等）
  - CLI基盤・テストフレームワーク

#### 候補となる統計法則
1. **ベンフォードの法則** ✅ 実装済み - 先頭桁分布
2. **ジップの法則** - 単語頻度分布（文書分析）
3. **ニューカムの法則** - 数値分布の別角度検証
4. **パレートの法則** - 80/20分布（所得・販売データ）
5. **ヒープスの法則** - 語彙成長率（文書解析）
6. **ロトカの法則** - 著者生産性分布（研究不正）
7. **ブラッドフォードの法則** - 文献分散（引用操作）
8. **メトカーフの法則** - ネットワーク価値（SNS分析）
9. **摩擦の法則** - 取引量・距離関係（商取引）
10. **べき分布則** - 都市人口・企業規模分布
...
#### 期待される効果
- **学習効果**: 一つのツールで多様な不正検知手法習得
- **相乗効果**: 複数法則による精度向上・偽陽性削減
- **専門分野特化**: 文書分析・財務・選挙・研究等の特化機能
- **エコシステム**: 統計法則コミュニティのプラットフォーム化
...
|--------|------------|------|-------------|
| ⭐⭐⭐⭐ | **ベンフォードの法則** | 財務監査、異常検知 | 高速・軽量・汎用性あり |
| ⭐⭐⭐⭐ | **Zipfの法則** | テキスト品質、自然性評価 | NLP系ツールと相性良し |
| ⭐⭐⭐⭐ | **パレート法則（80/20）** | 売上分析、リソース最適化 | ビジネス分析に強い |
| ⭐⭐⭐⭐ | **正規分布（ガウス）** | 外れ値検出、品質管理 | 統計処理の基本 |
| ⭐⭐⭐⭐ | **KS検定** | 分布との一致度評価 | 汎用的な異常検知に強い |
| ⭐⭐⭐⭐ | **自己相関** | 時系列解析 | ログ・センサーデータに有用 |
| ⭐⭐⭐⭐ | **分散分析（ANOVA）** | グループ比較 | 実験・A/Bテストに強い |
| ⭐⭐⭐ | **ポアソン分布** | イベント頻度、故障予測 | ログ解析に有用 |
| ⭐⭐⭐ | **エントロピー** | 暗号性、情報量評価 | セキュリティ・圧縮分析に強い |
| ⭐⭐⭐ | **PCA** | 次元削減 | 機械学習前処理に有用 |
| ⭐⭐ | **ローレンツ曲線・ジニ係数** | 不平等度分析 | 経済・社会データ向け |
| ⭐⭐ | **フラクタル次元** | 自己相似性、画像解析 | 専門性高め、ニッチ用途 |
| ⭐⭐ | **べき乗則（Power Law）** | ネットワーク、人気分布 | 可視化と組み合わせると強力 |
| ⭐⭐ | **クラスタリング** | グループ化 | GUI偏重の分野をCLI化できる |

##### 統合コマンド方式の設計思想
**コマンド名**: `lawkit` (law + toolkit = 自然法則・統計法則を扱うツールキット)

**利点**:
1. **発見性と普及力**: AIや人間が「lawkit」という1つのコマンドを覚えれば全法則にアクセス可能
2. **UNIX哲学との折衷**: コマンドは1つだが、サブコマンドごとに明確な責務を持つ
3. **インストールの簡便さ**: `cargo install lawkit`で全機能一括導入
4. **引数の統一**: `--format`, `--threshold`, `--summary`等のオプション共通化

##### AI時代のファイル形式対応戦略
**対応予定ファイル形式**:
| 形式 | 内容 | 対応方法（Rust） |
|------|------|------------------|
| .npy | NumPy配列 | ndarray-npy Crateで読み込み |
| .npz | 複数のNumPy配列 | 同上、zip展開して処理 |
| .pt | PyTorchテンソル | tch-rs Crateで読み込み（要libtorch） |
| .onnx | ONNXモデル | tract-onnxで読み込み、重み抽出 |
| .h5 | HDF5形式 | hdf5 Crateで読み込み（要libhdf5） |

**価値**: AI開発者のデータは構造化されており、統計法則の適用対象として理想的。CLIでの分析は未整備分野。

##### 他ツール連携戦略
**diffx連携例**:
```bash
# ベンフォード分析結果の比較
lawkit benf before.xlsx --format json > before.json
lawkit benf after.xlsx --format json > after.json
diffx before.json after.json  # 異常の変化を検出
```

**宣伝効果**: lawkitの出力形式一貫性をアピールしつつ、diffxの存在を自然に紹介
...
##### benf設計の継承戦略
**基本方針**: benfの洗練された引数設計・CLI体験を最大限活用

**理想的なコマンド形式**:
```bash
# 既存benf機能（完全互換）
lawkit benf data.xlsx --format json --filter ">=100" --threshold high

# 新法則（同じ引数体系）
lawkit zipf document.txt --format json --verbose
lawkit pareto sales.csv --format csv --filter ">=1000"
lawkit kstest data.xlsx --format yaml --threshold medium
```

**継承する設計要素**:
- `--format`: text/json/csv/yaml/toml/xml統一対応
- `--filter`: 数値範囲フィルタリング（適用可能な法則）
- `--threshold`: リスク評価閾値（適用可能な法則）
- `--lang`: 多言語出力（5言語対応）
- `--quiet/--verbose`: 出力詳細度制御
- `--min-count`: 最小データ数要件
- 国際数字処理エンジン（5文字体系対応）
- 終了コード体系（0/10/11）

##### diffx連携の具体化
**diffx**: Rust製オブジェクト比較特化ツール（開発予定）
...
xargs -I {} lawkit benf {} --format json | \
lawkit compare --input-format json > summary.json
```

**相乗効果**: JSON出力一貫性により、lawkit結果の比較・変化検出が自然に実現

##### フォルダ構成再編計画

**現在の問題**:
- 日本語数字変換等のbenf固有機能が混在
- 共通関数と法則固有関数が未分離
- スケーラビリティに課題
...
**法則別引数適用可能性**:
| 引数 | benf | zipf | pareto | kstest | 備考 |
|------|------|------|--------|--------|------|
| `--filter` | ✅ | ❌ | ✅ | ✅ | zipfは単語頻度のため数値範囲無意味 |
| `--threshold` | ✅ | ✅ | ✅ | ✅ | 全法則でリスク評価可能 |
| `--format` | ✅ | ✅ | ✅ | ✅ | 出力形式は全法則共通 |
| `--lang` | ✅ | ✅ | ✅ | ✅ | 多言語出力は全法則対応 |
| `--min-count` | ✅ | ✅ | ✅ | ✅ | 最小データ数は統計的に重要 |

**対処方針**:
- 無意味な引数は警告表示後に無視
- 法則固有引数は`--law-specific-*`形式で追加
- ヘルプで法則別適用可能引数を明示
...
**冒頭戦略（AIニーズ訴求）**:
```markdown
# lawkit: AI時代の統計法則ツールキット

## なぜ今、CLI統計ツールが必要なのか？

### 🤖 AI・自動化時代の要請
- **AIエージェントはCLIを操作できる**: GUI依存ツールは自動化困難
- **大規模データ処理**: findコマンドで数万ファイルを一括処理可能
- **パイプライン統合**: Unix哲学に基づく柔軟な組み合わせ

### 📊 既存ツールからの脱却
- **Excel/SPSS**: マウス操作必須、スクリプト化困難、高コスト
- **Python/R**: 重い依存関係、複雑なセットアップ、メモリ消費大
- **lawkit**: 軽量バイナリ、依存なし、高速・省メモリ
```

**パイプライン連携例**:
```bash
# 複数法則による多角的分析
cat financial-data.csv | \
lawkit benf --format json | \
jq '.risk_level' | \
lawkit pareto sales.csv --format json | \
diffx --compare-risk-levels

# 大規模監査ワークフロー
find ./audit-data -name "*.xlsx" | \
parallel lawkit benf {} --format json | \
lawkit aggregate --input-format json | \
lawkit report --output summary.html
```

##### 技術的課題と解決策

**課題1: サブコマンド実装の複雑性**
- 解決策: clapのサブコマンド機能活用、共通引数の継承システム

**課題2: 法則固有機能の分離**
- 解決策: trait-based設計で法則インターフェース統一

**課題3: 出力フォーマット一貫性**
- 解決策: 共通のResult構造体、統一JSON schema

**課題4: パフォーマンス（複数法則実行）**
- 解決策: 並列処理、メモリ効率化、キャッシュ戦略