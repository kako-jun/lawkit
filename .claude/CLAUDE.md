...
- [ ] **OpenDocument**: Spreadsheet (.ods), Text (.odt)  
- [ ] **PDF**: 文書解析 (.pdf)
- [ ] **構造化データ**: CSV/TSV, JSON/XML, YAML/TOML（入力として）
- [ ] **HTML**: Webページ（.html）

**技術的準備状況**: 
- 必要なクレートは全てCargo.tomlに追加済み
- calamine (Excel/ODS), pdf-extract (PDF), docx-rs (Word), scraper (HTML)
- 実装は主にファイル形式判定→適切なパーサー呼び出し
...
#### コード品質
- 未使用関数警告: 1件（parse_kanji_number in japanese.rs）
- テストカバレッジ: 良好（統合テスト100%通過）
...
#### 次のステップ
- **即座に開始可能**: 入力形式拡張（依存関係準備済み）
- **段階的実装**: Excel → PDF → CSV → その他の順序推奨
- **低リスク**: 既存機能への影響最小限