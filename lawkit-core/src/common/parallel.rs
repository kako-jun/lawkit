    ...
    F: Fn(&T) -> U + Send + Sync + Copy + 'static,
{
    ...
    let chunks: Vec<Vec<T>> = owned_data.chunks(config.chunk_size)
    ...
    for handle in handles {
        handle.join().map_err(|_| {
            crate::error::BenfError::ParseError("Thread join failed".to_string())
        })?;
    }
    ...
    R: Fn(U, U) -> U + Send + Sync + 'static,
{
    ...
    // 結果をリダクション
    let final_result = map_result.results.into_iter()
        .fold(initial, reduce_func);
...
pub fn parallel_statistics(
    ...
    let chunks: Vec<Vec<f64>> = owned_data.chunks(config.chunk_size)
        .map(|chunk| chunk.to_vec())
        .collect();
    
    // 各チャンクで統計を計算
    let chunk_stats = parallel_map(
        &chunks,
        config,
        |chunk| calculate_chunk_statistics(chunk),
    )?;
...
pub fn parallel_benford_analysis(
    ...
    let chunk_results = parallel_map(
        &chunks,
        config,
        |chunk| analyze_benford_chunk(chunk),
    )?;
...
pub fn parallel_outlier_detection(
    ...
    // データを所有版に変換してチャンクごとに分割
    let owned_data: Vec<f64> = data.to_vec();
    let chunks_with_offset: Vec<(usize, Vec<f64>)> = owned_data.chunks(config.chunk_size)
        .enumerate()
        .map(|(chunk_idx, chunk)| (chunk_idx * config.chunk_size, chunk.to_vec()))
        .collect();
    
    let outlier_results = parallel_map(
        &chunks_with_offset,
        config,
        move |(offset, chunk)| detect_outliers_chunk(chunk, *offset, mean, std_dev, z_threshold),
    )?;
