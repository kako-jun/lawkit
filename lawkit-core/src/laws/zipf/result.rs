...
impl ZipfResult {
    pub fn new(dataset_name: String, frequencies: &[f64]) -> Result<Self> {
        ...
        // 分布品質スコア（Zipf法則への適合度）
        let distribution_quality = calculate_distribution_quality(zipf_exponent, correlation_coefficient);
        ...
        // リスクレベル判定
        let risk_level = determine_risk_level(zipf_exponent, correlation_coefficient, distribution_quality);
...
fn calculate_distribution_quality(zipf_exponent: f64, correlation: f64) -> f64 {
    ...
    // 重み付き平均
    (exponent_score * 0.6 + correlation_score * 0.4).max(0.0).min(1.0)
}
